...
# Авторство
echo "Протестированно на Ubuntu 24.04 LTS"
echo "Автор: ilef-dev"
...
prompt_update() {
    read -p "Рекомендуем использовать этот скрипт на чистой Ubuntu-Server и обновить пакеты перед запуском. Хотите продолжить? (y/n): " response
    case "$response" in
        [yY][eE][sS]|[yY])
            ;;
        [nN][oO]|[nN])
